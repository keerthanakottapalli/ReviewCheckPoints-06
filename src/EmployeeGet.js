...
export default function EmployeeReviews() {
    ...
    const handleDownload = async (imageUrl) => {
        try {
            ...
            const filenameWithEncoding = imageUrl.split('/').pop().split('?')[0];
            ...
            if (response.ok) {
                ...
                console.error('Failed to fetch image from the API endpoint');
            }
        } catch (error) {
                ...
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
                    <Card style={{ display: 'flex', flexDirection: 'column', backgroundColor: '#e9ecef' }}>
                        <span style={{ top: 0, right: 0 }}>
                            <Button style={{ fontSize: '26px', color: 'black' }} onClick={handlePreviewModalClose}>
                                X
                            </Button>
                        </span>
                        <CardContent>
